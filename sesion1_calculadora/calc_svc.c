...
static void
nprog_1(struct svc_req *rqstp, register SVCXPRT *transp)
{
	union {
		datos suma_1_arg;
		datos resta_1_arg;
...
main (int argc, char **argv)
{
	...
	if (!svc_register(transp, NPROG, NVERS, nprog_1, IPPROTO_UDP)) {
		fprintf (stderr, "%s", "unable to register (NPROG, NVERS, udp).");
		exit(1);
	}
	...
	if (!svc_register(transp, NPROG, NVERS, nprog_1, IPPROTO_TCP)) {
		fprintf (stderr, "%s", "unable to register (NPROG, NVERS, tcp).");
		exit(1);
	}
